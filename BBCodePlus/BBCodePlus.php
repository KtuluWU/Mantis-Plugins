	...
	class BBCodePlusPlugin extends MantisFormattingPlugin {
		...
		public function config() {
			...
			return array(
				...
				'code_name_alias'  => array( 
											'c#' => 'csharp',
											),
				'language_valid_list' => array ( "markup" )	
			);
		}
		//-------------------------------------------------------------------
		/**
		...
		function string_process_bbcode( $p_string, $p_multiline = TRUE ) {
			...
			*/
			...
			$p_string = preg_replace_callback('/\[code=([^\]\s]+)?\s+start=([0-9]+)\s*\](.+)?\[\/code\]/imsU', 
							function ($p_matches){
								return "<pre class=\"line-numbers\" data-start=\"" . $p_matches[2] . "\"><code class=\"language-" . $this->language_validate( $p_matches[1] ) . "\">" . $p_matches[3] . "</code></pre>"; 
							} 
							, $p_string
						);

			$p_string = preg_replace_callback('/\[code=([^\]\s]+)?\s*\](.+)?\[\/code\]/imsU', 
							function ($p_matches){
								return "<pre><code class=\"language-" . $this->language_validate( $p_matches[1] ) . "\">" . $p_matches[2] . "</code></pre>"; 
							}, 
							$p_string
						);
		...
		//-------------------------------------------------------------------
		/* FWA Modification */
		function language_validate( $p_matches_name ) {
			$t_language_name = strtolower( $p_matches_name );
			$t_language_name_alias = plugin_config_get( 'code_name_alias' );

			if ( array_key_exists($t_language_name, $t_language_name_alias) ) {
				$t_language_name = $t_language_name_alias[ $t_language_name ];
			}

			if ( 0 != preg_match('/[^_a-zA-Z0-9-]+/i', $t_language_name ) && !array_key_exists($t_language_name, $t_language_name_alias)) {
				$t_language_name = "none";
			}
